...
public class AssessmentServiceimp implements AssementService{
    ...
    @Override
    public Assessment updateAssessment(Long assessmentId, Assessment tmpAssessment) throws AssessmentNotFoundException {
        ...
        if (optionalAssessment.isPresent()) {
            ...
            assessment.setCourseName(tmpAssessment.getCourseName());
            assessment.setAssessmentType(tmpAssessment.getAssessmentType());
            assessment.setPassingPercentage(tmpAssessment.getPassingPercentage());
            assessment.setNumberOfViolations(tmpAssessment.getNumberOfViolations());
            return assessmentRepository.save(assessment);
        } else {
            throw new AssessmentNotFoundException("Assessment not found with this id: " + assessmentId);
        }
    }
    ...
    //List of Assessment that are active and upcoming
    @Override
    public List<Assessment> getActiveAndUpcomingAssessments() {
        ...
        allAssessments.sort(Comparator.comparingLong(Assessment::getAssessmentId).reversed());
        return allAssessments;
    }

    //List of Assessment that are completed

    @Override
    public List<Assessment> getAllCompletedAssessmensts() {
        LocalDateTime currentTime = LocalDateTime.now();
        return assessmentRepository.findCompletedAssessments(currentTime);

    }

    //De-Activate Assessment by ID
    ...
    @Override
    public Optional<Assessment> getAssessmentById(Long id) throws AssessmentNotFoundException {
        Optional<Assessment> tmpAssessment =assessmentRepository.findById(id);
        if(tmpAssessment.isPresent()) {
            return tmpAssessment;
        }
        else {
    ...
    @Override
    public List<String> getTechnologyByBatch(String targetbatch) {
        return assessmentRepository.getTechnologyByBatch(targetbatch);
    }
